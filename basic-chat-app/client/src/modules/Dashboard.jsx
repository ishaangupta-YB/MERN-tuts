...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchConversations = async () => {
      const res = await fetch(
        `${API_URL}/api/conversations/${loggedInUser?.id}`,
        {
          method: "GET",
          headers: {
  ...
  const fetchMessages = async (conversationId, receiver) => {
		const res = await fetch(`${API_URL}/api/message/${conversationId}?senderId=${user?.id}&&receiverId=${receiver?.receiverId}`, {
			method: 'GET',
			headers: {
				'Content-Type': 'application/json',
			}
